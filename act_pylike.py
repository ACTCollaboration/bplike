...
class StevePower(object):
    def __init__(self,froot,flux,infval=1e10,tt_lmin=600,tt_lmax=None):
        ...
        cov =np.loadtxt(f'{froot}coadd_cov_{flux}_200116.txt')
...
class act_pylike(_InstallableLikelihood):
    ...
    def initialize(self):
        ...
        self.fparams = utils.config_from_yaml('params.yml')['fixed']
        self.aparams = utils.config_from_yaml('params.yml')['act_like']
        self.bpmodes = utils.config_from_yaml('params.yml')['bpass_modes']
    ...
    def prepare_data(self, verbose=False):
        flux = self.flux
        self.sp = StevePower("data/actpol_2f_full_s1316_2flux_fin/data/data_act/ps_200116/",self.flux)
        if self.bandpass:
        ...
        self.fgpower = afg.ForegroundPowers(self.fparams,self.sp.ells,
...
class act15(act_pylike):
    flux = '15mJy'

class act100(act_pylike):
    flux = '100mJy'
